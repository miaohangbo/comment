 ...
 */
public class FrameUtilV2 {
	private static final Logger LOGGER = LoggerFactory.getLogger(FrameUtilV2.class);

	private static TokenService tokenService = SpringBeanUtil.getBean("tokenService");
	private static final String USERID_KEY = "beijIng5ssdfhh5521dfhslanxiong";
	private static final String ENCRYPT_KEY = "DG43AE2F9DCKLD32F8";
	...
	public static String encryptionuid(Long userid)
		...
		String str = encryption(userid + "|" + new Date().getTime(), FrameUtilV2.USERID_KEY);
		if (str != null) {
			if (str.indexOf("+") >= 0) {
				str = str.replaceAll("\\+", "*");
			}
	...
	public static String decryptionuid(String s) throws IOException {
		...
		String str = decryption(s, FrameUtilV2.ENCRYPT_KEY);
...
//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		...
		/*try {
			...
			String str = decryption(token, FrameUtil.USERID_KEY);
			if (isNullorBlank(str)) {
				return null;
			}
			StringTokenizer st = new StringTokenizer(str, "|");
	...
	public static String getToken(Long userid, String passwd) {
		...
		String str = null;
		try {
			str = encryption(userid + "|" + passwd + "|" + new Date().getTime(), FrameUtilV2.USERID_KEY);
...
//		System.out.println(getUserid("AB1lEcMSv2W3YRvUfEN8A1VKc4szZNHpi8BapmIYNXwVDw=="));
	}
}